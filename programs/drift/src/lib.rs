...
pub mod drift {
    ...
    // ── Nebula DEX: ZK Commitment Instructions ────────────────────────────

    pub fn open_perp_with_commitment<'c: 'info, 'info>(
        ctx: Context<'_, '_, 'c, 'info, OpenPerpWithCommitment<'info>>,
        market_index: u16,
        collateral_amount: u64,
        is_long: bool,
        commitment: [u8; 32],
    ) -> Result<()> {
        handler_open_perp_with_commitment(ctx, market_index, collateral_amount, is_long, commitment)
    }

    pub fn close_perp_with_reveal<'c: 'info, 'info>(
        ctx: Context<'_, '_, 'c, 'info, ClosePerpWithReveal<'info>>,
        market_index: u16,
        ...
        handler_close_perp_with_reveal(
            ctx,
            market_index,
    ...
    pub fn perp_shield_liquidation_check<'c: 'info, 'info>(
        ctx: Context<'_, '_, 'c, 'info, PerpShieldLiquidationCheck<'info>>,
        revealed_size: u64,
        revealed_leverage_bps: u16,
        revealed_entry_price_x64: u128,
        revealed_liquidation_price_x64: u128,
        ...
        handler_perp_shield_liquidation_check(
            ctx,
            revealed_size,
            revealed_leverage_bps,
            revealed_entry_price_x64,
